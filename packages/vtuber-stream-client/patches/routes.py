...
def init_client_ws_route(default_context_cache: ServiceContext) -> APIRouter:
    ...
    async def speak_status():
        ...
        return JSONResponse({"clients": len(ws_handler.client_connections)})

    @router.post("/api/speak")
    async def speak_endpoint(request: Request):
        """
        Accept text, generate TTS, and broadcast audio to all connected
        WebSocket clients so the avatar lip-syncs.
        """
        ...
        logger.info(f"[/api/speak] Generating TTS for: {text[:80]}...")

        try:
            # Generate TTS audio file
            file_name = f"speak_{datetime.now().strftime('%Y%m%d_%H%M%S')}_{str(uuid4())[:8]}"
            audio_path = await default_context_cache.tts_engine.async_generate_audio(
                text=text, file_name_no_ext=file_name
            )
            logger.info(f"[/api/speak] Audio generated: {audio_path}")

            # Build the audio payload with lip-sync volumes
            payload = prepare_audio_payload(
                audio_path=audio_path,
                display_text={"name": "Narrator", "text": text},
                actions=None,
                forwarded=False,
            )
            ...
            for uid, ws in list(ws_handler.client_connections.items()):
                try:
                    ...
                    logger.warning(f"[/api/speak] Failed to send to {uid}: {e}")

            logger.info(f"[/api/speak] Broadcast to {sent_count} client(s)")
            return JSONResponse({"ok": True, "clients": sent_count})

        except Exception as e:
            logger.error(f"[/api/speak] Error: {e}")
