...
# Retry up to MAX_RETRIES times — the server may not be ready for game connections
# immediately (RCON readiness ≠ game-port readiness), and the client loads fast.
MAX_RETRIES=8
RETRY_DELAY=5

for attempt in $(seq 1 $MAX_RETRIES); do
    log "Attempt ${attempt}/${MAX_RETRIES}: connecting to ${SERVER_HOST}:${SERVER_PORT}..."
    START_TS=$(date +%s)
    ...
    EXIT_CODE=$?
    SESSION_SECS=$(( $(date +%s) - START_TS ))
    log "Factorio exited (code=${EXIT_CODE}, ran=${SESSION_SECS}s)"

    # A session shorter than 60s is almost certainly a failed connection attempt.
    # A longer session means we actually played and the server closed normally.
    if [ "$SESSION_SECS" -ge 60 ]; then
        log "Session ended normally after ${SESSION_SECS}s"
        exit 0
    fi

    if [ "$attempt" -lt "$MAX_RETRIES" ]; then
        log "Short session — connection likely refused. Retrying in ${RETRY_DELAY}s..."
        sleep "$RETRY_DELAY"
    fi
done
